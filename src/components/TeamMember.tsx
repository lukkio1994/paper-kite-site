 ...
 * 
 * Reusable component for displaying team member information with social links.
 * Makes it easy to add new team members by simply adding their data to the translations.
 * 
 * Features:
 * - Gradient avatar with initials
 * - Name, role, and description
 * - Social media links (LinkedIn, Twitter/X, Discord, Instagram)
 * - Hover effects and accessibility features
 * - Responsive design
 * 
 * Usage:
 * <TeamMember memberKey="david" />
 * <TeamMember memberKey="luis" />
 * <TeamMember memberKey="carlos" />
 */
...
const TeamMember = ({ 
  ...
  const socialColors = {
    ...
    twitter: 'hover:text-black dark:hover:text-white',
  ...
  return (
    <div className="text-center">
      <div className={`w-32 h-32 bg-gradient-to-br from-${gradientFrom} to-${gradientTo} rounded-full mx-auto mb-4 flex items-center justify-center`}>
        <span className="text-white font-bold text-xl">
          {t(`team.members.${memberKey}.initials`)}
        </span>
      </div>
      ...
      <p className="text-blue-600 dark:text-blue-400 mb-2">
        {t(`team.members.${memberKey}.role`)}
      </p>
      <p className="text-gray-600 dark:text-gray-300 text-sm mb-4">
        {t(`team.members.${memberKey}.description`)}
      </p>
      
      {/* Social Links */}
      <div className="flex justify-center space-x-3">
        {Object.entries(socialIcons).map(([platform, icon]) => {
          const socialUrl = t(`team.members.${memberKey}.socials.${platform}`);
          const isValidUrl = socialUrl && socialUrl !== '' && socialUrl !== '#';
          
          // Map platform names to proper display names for aria-labels
          const platformNames = {
            linkedin: 'LinkedIn',
            twitter: 'Twitter',
            discord: 'Discord',
          ...
          return (
            <a 
              key={platform}
              href={socialUrl || '#'} 
              className={`text-gray-400 ${socialColors[platform as keyof typeof socialColors]} transition-colors ${!isValidUrl ? 'cursor-not-allowed opacity-50' : ''}`}
              aria-label={`${t(`team.members.${memberKey}.name`)} ${platformNames[platform as keyof typeof platformNames]}`}
